...
export class TicketBotServer implements ITicketsServer {
        ...
        async (req: ServerUnaryCall<OpenTicketRequest>, callback: sendUnaryData<TicketData>) => {
            ...
            try {
                const ticket = await prisma.$transaction(async (tx) => {
                    ...
                    const userTickets = await tx.supportTicket.count({
                        where: {
                            userId: userId,
                            status: 'open',
                    ...
                    if (userTickets !== 0) {
                        throw new Error('TICKET_ALREADY_OPENED');
                    }

                    const freeAgent = callcenter.agents.find(a => !a.activeTicket);
                    ...
                    if (freeAgent) {
                        const createdTicket = await tx.supportTicket.create({
                            data: {
                                ...
                                findTime: new Date(),
                                status: 'open',
                                agentKey: freeAgent.id,
                                callCenterKey: callcenter.id,
                                username: username
                            }
                        });

                        const agent = await tx.supportAgent.update({
                            where: {
                                id: freeAgent.id
                            },
                            data: {
                                activeTicketKey: createdTicket.id
                            }
                        });
        ...
        async (req: ServerUnaryCall<GetStatusRequest>,
            ...
            const response = await prisma.$transaction(async tx => {
                ...
                if (agent) {
                    ...
                    if (agent.activeTicket) {
                        response.setSupportstate(SupportState.TICKET_OPEN);

                        const ticket = new TicketData();
                        ticket.setAgentid(agent.userId);
                        ticket.setTicketid(agent.activeTicket.id);
                        ticket.setUserid(agent.activeTicket.userId)
                        ticket.setUsermessages(agent.activeTicket.userMessages);
                        ticket.setAgentmessages(agent.activeTicket.agentMessages);
                        ticket.setUsername(agent.activeTicket.username ?? '');

                        response.setTicketdata(ticket);
                    } else {
                        response.setSupportstate(SupportState.IDLE);
                        response.setEmpty(new Empty());
                    }
                } else {
                    response.setUserstate(UserState.USER);
                    ...
                    if (ticket) {
                        const ticketR = new TicketData();
                        ticketR.setUserid(userId);
                        ticketR.setTicketid(ticket.id);
                        ticketR.setUsermessages(ticket.userMessages);
                        ticketR.setAgentmessages(ticket.agentMessages);
                        ticketR.setUsername(ticket.username ?? '');

                        if (ticket.activeAgent) {
                            response.setSupportstate(SupportState.TICKET_OPEN);
                            ticketR.setAgentid(ticket.activeAgent.userId);
                        } else {
                            response.setSupportstate(SupportState.TICKET_PENDING_ANSWER);
                            ticketR.setEmpty(new Empty());
                        }

                        response.setTicketdata(ticketR);
                    } else {
                        response.setSupportstate(SupportState.IDLE);
                        response.setEmpty(new Empty());
                    }
